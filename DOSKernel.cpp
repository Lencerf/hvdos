...
static inline void ConvertSlashes(std::string &S) {
  std::transform(S.begin(), S.begin(), S.end(),
                 [](char C) { return (C == '\\') ? '/' : C; });
}

static inline uint8_t ModeToAttribute(uint16_t Mode) {
  uint8_t Attribute = 0;
...
DOSKernel::DOSKernel(char *memory, hv_vcpuid_t vcpu, int argc, char **argv)
    : _memory(memory), _vcpu(vcpu), _dta(0), _exitStatus(0) {
  _fdbits.resize(256);
...
int DOSKernel::dispatch(uint8_t IntNo) {
  switch (IntNo) {
    case 0x20:
      return int20();
    case 0x21:
...
int DOSKernel::int21Func0E() {
  SET_AL(DL + 'A');
  return STATUS_HANDLED;
}

// DOS 1+ - GET CURRENT DEFAULT DRIVE
int DOSKernel::int21Func19() {
  SET_AL(0);
  return STATUS_HANDLED;
}

// DOS 1+ - SET DISK TRANSFER AREA ADDRESS
int DOSKernel::int21Func1A() {
  _dta = DX;
  return STATUS_HANDLED;
}

void DOSKernel::makePSP(uint16_t seg, int argc, char **argv) {
  ...
  for (int i = 2; i < argc && c < 0x7E; i++) {
    j = 0;
    PSP->CommandLine[c++] = ' ';
    while (argv[i][j] && c < 0x7E) PSP->CommandLine[c++] = argv[i][j++];
  }
...
int DOSKernel::int21Func26() {
  makePSP(DS, 0, 0);
  return STATUS_HANDLED;
}

// DOS 2+ - GET DOS VERSION
int DOSKernel::int21Func30() {
  SET_AL(7);
  SET_AH(0);
  return STATUS_HANDLED;
}
...
int DOSKernel::int21Func4C() {
  _exitStatus = AL;
  return STATUS_STOP;
}

// DOS 2+ - FINDFIRST - FIND FIRST MATCHING FILE
int DOSKernel::int21Func4E() {
...
int DOSKernel::getDOSError() const {
  // TODO translate
  return errno;
}

void DOSKernel::flushConsoleInput() { fflush(stdout); }

int DOSKernel::internalGetChar(bool Echo) { return getchar(); }

int DOSKernel::allocFD(int HostFD) {
  auto I = std::find(_fdbits.begin(), _fdbits.end(), false);
  if (I == _fdbits.end()) return -1;
...
void DOSKernel::deallocFD(int FD) {
  if (FD < 3) return;
...
void DOSKernel::writeMem(uint16_t const &Address, void const *Bytes,
                         size_t Length) {
  auto D = Address;
...
std::string DOSKernel::readString(uint16_t const &Address, size_t Length) {
  std::string Result;
  auto S = Address;
...
std::string DOSKernel::readCString(uint16_t const &Address, char Terminator) {
  std::string Result;
  auto S = Address;
